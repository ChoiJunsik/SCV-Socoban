...
void undo_load(int stage)
{
  ...
  if(undoload_count>5)
              exit(0);//5번하면 언두못함
  for (int i=0; i<30; i++)  //맵 저장
      for (int j=0; j<30; j++)
        map_now[stage][i][j]=undo[undo_count][i][j];
  undo_count--;
  return;
}


void undo_save(int stage)
{
  for (int i=0; i<30; i++)  //맵 저장
      for (int j=0; j<30; j++)
        undo[undo_count][i][j]=map_now[stage][i][j];
if(undo_count<5)
    undo_count++;
else if(undo_count==5){
    for(int k=0;k<5;k++)
      for (int i=0; i<30; i++)  //맵 저장
          for (int j=0; j<30; j++)
            undo[k][i][j]=undo[k+1][i][j];
    undo_count=4;}
